    ...
    if case == "none":
        ...
        for word in words:
            ...
            if word.lstrip(NUMERALS + PUNCT).casefold() in case_sensitive_folded:
                i = case_sensitive_folded.index(
                    word.lstrip(NUMERALS + PUNCT).casefold()
                )
                sc_line.append(
                    word[: -len(case_sensitive_units[i])] + case_sensitive_units[i]
                )
                continue
            ...
            if case == "sentence" and word.casefold() in case_sensitive_folded:
                sc_line.append(
                    case_sensitive[case_sensitive_folded.index(word.casefold())]
                )
                continue
            ...
            is_all_consonants = re.search(
                "\A[" + CONSONANTS + PUNCT + NUMERALS + "]+\Z",
                word,
                flags=re.IGNORECASE,
            )
...
def main():
    ...
    editor.appendText(
        "# Auto-generated by {}\r\n# Imported from {}\r\n".format(
            __file__[__file__.rfind("\\") + 1 :], spec_file
        )
    )
        ...
        with docx.open("word/document.xml") as doc:
            ...
            for p in root.findall(".//w:p", ns):
                ...
                if h1 is not None:
                    ...
                    for t in p.findall(
                        ".//w:pPr/w:pStyle[@w:val='Heading1']/../../w:r/w:t", ns
                    ):
                ...
                for t in p.findall(
                    ".//w:pPr/w:pStyle[@w:val='Heading3']/../../w:r/w:t", ns
                ):
                ...
                if cur_section_no and selections_section and text:
                    ...
                    editor.appendText(
                        "{0} {1}\t{0}\r\n".format(
                            cur_section_no, text.replace(" ", "\t", 1).strip()
                        )
                    )
