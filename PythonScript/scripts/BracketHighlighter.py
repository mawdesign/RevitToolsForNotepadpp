...
try:
    ...
    def indicatorOptionsSet(
        ...
        which_editor.indicSetStyle(
            indicator_number, indicator_style
        )  # e.g. INDICATORSTYLE.ROUNDBOX
    ...
    for editorX in (editor1, editor2):
        ...
        indicatorOptionsSet(
            BH__dict["indic_for_box_at_caret"],
    ...
    def BH__containing_box_indices_into_string(
        str_containing_caret, caret_index_into_str
    ):
        class Stack:
            def __init__(self):
                self.clear()
        ...
        get_closing_char_via_opening_char_dict = dict(
            (v, k) for (k, v) in get_opening_char_via_closing_char_dict.items()
        )
        ...
        if box_ending_index != -1:
            if box_starting_index != -1:
                if (
                    str_containing_caret[box_ending_index]
                    == get_closing_char_via_opening_char_dict[
                        str_containing_caret[box_starting_index]
                    ]
                ):
                    retval = (
                        len(str_containing_caret[:box_starting_index].encode("utf-8")),
                        len(
                            str_containing_caret[: box_ending_index + 1].encode("utf-8")
                        ),
                    )
    ...
    def BH__fileIsCloned(file_name_to_test):
        ...
        if file_name_to_test in clone_detect_dict:
            if clone_detect_dict[file_name_to_test] >= 1:
                retval = True
        return retval
    ...
    def BH__getViewableEditorAndRangeTupleListList(work_across_both_views):
        ...
        def consolidate_range_tuple_list(range_tup_list):
            sorted_range_tup_list = sorted(
                range_tup_list
            )  # sort criteria is first element of tuple in list
        ...
        def get_onscreen_pos_tup_list(
            which_editor,
        ):  # which_editor is editor1 or editor2 (or maybe even just plain editor)
            ...
            while currentLine < lastLine:
                ...
                if docLine != prevDocLineChecked:
                    ...
                    if endPos == -1:
                        ...
                        break_out = True
                    if endPos > startPos:
                        temp_tup_list.append((startPos, endPos))
                    if break_out:
                        break
                currentLine += 1
        ...
        curr_file_active_in_both_views = (
            BH__fileIsClonedAndIsActiveInBothViews(notepad.getCurrentFilename())
            if both_views_open
            else False
        )
        if both_views_open:
        ...
        if curr_file_active_in_both_views:
            range_tup_list = list(
                consolidate_range_tuple_list(ed1_range_tup_list + ed2_range_tup_list)
            )
    ...
    def BH__callback_sci_UPDATEUI(args):
        ...
        if args["updated"] == UPDATE.CONTENT and BH__dict[
            "last_modificationType_for_hack"
        ] == (MODIFICATIONFLAGS.CHANGEINDICATOR | MODIFICATIONFLAGS.USER):
            return

        for editorX, pos_range_tuple_list in BH__getViewableEditorAndRangeTupleListList(
            True
        ):
            ...
            for start_pos, end_pos in pos_range_tuple_list:
                if start_pos <= editorX.getCurrentPos() <= end_pos:
                    (
                        box_start_offset,
                        box_end_offset,
                    ) = BH__containing_box_indices_into_string(
                    ...
                    if box_start_offset != None:
                        ...
                        if size_of_box_in_chars <= 2:
                            ...
                            editorX.setIndicatorCurrent(
                                BH__dict["indic_for_box_at_caret"]
                            )
                            editorX.indicatorFillRange(
                                start_pos + box_start_offset, size_of_box_in_chars
                            )

    editor.callbackSync(
        BH__callback_sci_UPDATEUI, [SCINTILLANOTIFICATION.UPDATEUI]
    )  # install callback
    editor.callbackSync(
        BH__callback_sci_MODIFIED, [SCINTILLANOTIFICATION.MODIFIED]
    )  # may not need to be "Sync", but for now we'll make it that way

else:
    editor.setSelectionMode(
        editor.getSelectionMode()
    )  # force manual UPDATEUI to happen
