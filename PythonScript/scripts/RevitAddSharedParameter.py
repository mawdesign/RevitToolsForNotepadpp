...
def incStr(str):
    ...
    for i in range(len(str), 0, -1):
        ascii = ord(str[i - 1])
        if carry and ascii in list(range(ord("0"), ord("9") + 1)) + list(
            range(ord("A"), ord("Z") + 1)
...
def createParams(*args):
    ...
    if paramSettings["groupName"] in group.keys():
        ...
        if firstGroupLine != totalLines:
            ...
            editor.replaceWholeLine(
                lastGroupLine,
                editor.getLine(lastGroupLine)
                + "GROUP\t"
                + groupNo
                + "\t"
                + paramSettings["groupName"]
                + "\r\n",
            )
    ...
    for i in range(paramCount):
        ...
        paramLine = (
            ...
            + "\t"
            + paramSettings["name"]
            + paramSettings["suffix"]
            + "\t"
            + paramSettings["type"]
            + "\t\t"
            + paramSettings["group"]
            + "\t"
            + paramSettings["visible"]
            + "\t"
            + paramSettings["desc"]
            + "\t1\t"
            + paramSettings["hide"]
...
pType_tip = Hovertip2(
    pType_add,
    "Show less frequently used \nparameter types in the list",
    hover_delay=100,
)
...
pVisible_check = ttk.Checkbutton(
    mainframe, text="", variable=pVisible, onvalue="1", offvalue="0"
)
...
pHide_check = ttk.Checkbutton(
    mainframe, text="", variable=pHide, onvalue="1", offvalue="0"
)
...
pIterate_spin = ttk.Spinbox(
    mainframe, width=5, from_=1, to=100, increment=1, textvariable=pIterate
)
...
pSuffix_tip = Hovertip2(
    pSuffix_entry,
    "This will be added to the name and \nincremented for each new parameter",
    hover_delay=100,
)
...
ttk.Button(mainframe, text="Close", command=closeDialog).grid(
